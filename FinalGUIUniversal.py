    ...
    def credentialcheck():
        ...
        if myresult != []:
            if (a,) not in myresult:
                if b == c:
                    try:
                        mycursor.execute(f"INSERT INTO login(username,password) VALUES('{a}','{b}')")
                        mycursor.execute(
                    ...
                    except sqlite3.connector.errors.IntegrityError:
            ...
            if b == c:
                ...
                mycursor.execute(f"INSERT INTO login(username,password) VALUES('{a}','{b}')")
                mycursor.execute(
                    f"CREATE TABLE IF NOT EXISTS {a} (sitename VARCHAR(255),mailID VARCHAR(255),password VARCHAR(255))")
                mydb.commit()
                lbl['text'] = 'SUCCESSFULLY SIGNED UP'
                lbl['font'] = ('BentonSans Comp Black', 12)
        ...
        if det in myresult:
            ...
            def add_pword():
                ...
                def credadd():
                    ...
                    det = (a, b, c)
                    if det != ('', '', ''):
                        mycursor.execute(f"INSERT INTO {username} (sitename,mailID,password) VALUES('{a}','{b}','{c}')")
                        mydb.commit()
                    else:
                        pass
            ...
            def update_pword():
                ...
                def update1():
                    ...
                    d = ent4.get()
                    f = ent6.get()
                    sqlFormula1 = (f"UPDATE {username} SET mailID= '{d}' WHERE sitename='{a}'")
                    sqlFormula2 = (f"UPDATE {username} SET password='{f}' WHERE sitename='{a}'")
            ...
            def importfromcsvfile():
                ...
                if file is not None and file != '':
                    ...
                    for i in readed:
                        mycursor.execute(f"INSERT INTO {username}(sitename,mailID,password) VALUES('{i[0]}','{i[1]}','{i[2]}')")
                        mydb.commit()
                    file1.close()
                else:
                    pass
